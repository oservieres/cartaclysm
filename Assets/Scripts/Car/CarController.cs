...
namespace Cartaclysm.Car
{
	...
	public class CarController : MonoBehaviour
	{
		...
		[SerializeField] private float m_MaxSpeedSteerAngle;
		[SerializeField] private float m_MinSpeedSteerAngle;
		...
		private void AdjustTorque(float forwardSlip)
		{
			...
			{
				m_CurrentTorque += 200 * m_TractionControl;
		...
		void OnCollisionEnter(Collision col) {
			if (col.gameObject.tag == "ChallengeHazard" && !isDead) {
				isDead = true;
				Die();
			}
			if (col.relativeVelocity.magnitude > 50 && !isDead) {
				isDead = true;
				Die();
			}
		}
